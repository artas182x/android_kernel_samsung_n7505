				      ...
				      struct mmc_async_req *next_req)
{
	...
	while (1) {
		...
		if (context_info->is_done_rcv) {
			...
			cmd = mrq->cmd;
			/* Apply : retrying CMD for Data CRC errors */
...
bool mmc_check_card_init(struct mmc_host *host)
{
	return host->host_flag & MMC_FLAG_INIT;
}
EXPORT_SYMBOL(mmc_check_card_init);
...
static int mmc_wait_trans_state(struct mmc_card *card, unsigned int wait_ms)
{
	...
	while (R1_CURRENT_STATE(status) != R1_STATE_TRAN) {
		if (waited > wait_ms) {
			pr_err("%s: card is not ready yet.(0x%x).\n",
...
int mmc_bkops_enable(struct mmc_host *host, u8 value)
{
	...
	err = mmc_wait_trans_state(card, 100);
	if (err < 0)
		goto bkops_out;

	err = mmc_send_ext_csd(card, ext_csd);
	if (err) {
		if (mmc_wait_trans_state(card, 300) >= 0) {
			err = mmc_send_ext_csd(card, ext_csd);
			if (err) {
